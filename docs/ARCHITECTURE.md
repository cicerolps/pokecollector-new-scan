...
4. `card_scan_hash.py` calcula três hashes perceptuais (phash/dhash/whash, via `imagehash`) da imagem normalizada — testando também as quatro rotações possíveis — e busca os `SCAN_HASH_TOP_N` candidatos mais próximos (padrão `10`) na tabela `card_hashes` por distância de Hamming combinada. Se o melhor candidato estiver além de `SCAN_HASH_NO_MATCH_DISTANCE`, o resultado já é `no_match` e nada mais roda.
5. `card_scan_ocr.py` lê, na mesma imagem (rotação vencedora do passo anterior), o número/total de coleção (faixa inferior direita), o ilustrador e um possível código de set (canto inferior esquerdo) e o nome da carta (faixa do título). Cada campo é opcional — a leitura de um não depende da dos outros.
6. `card_scan_resolver.py` usa esses campos para filtrar e reordenar deterministicamente os candidatos do hash: um número de coleção que não bate com a carta a **exclui** da lista inteiramente (mesma regra do antigo rankeador por metadados do Gemini); nome, ilustrador e código de set batendo **somam pontos** a favor do candidato (nunca excluem sozinhos, já que a leitura desses campos é mais sujeita a ruído do OCR). Quando os campos não decidem (nenhum foi lido, ou empatam entre os candidatos), a distância de hash resolve o empate — exatamente o comportamento anterior, preservado como fallback.
7. A confiança final é `confident` quando um único candidato sobra, quando os campos elegem um vencedor sozinhos (`hash_fields`), ou quando o hash decide um empate de campos com folga suficiente (`SCAN_HASH_CONFIDENCE_GAP`, decisão `hash`); caso contrário o resultado fica `ambiguous`, com os candidatos restantes (já filtrados pelos campos, quando aplicável) disponíveis para escolha manual.
8. Os resultados são persistidos na caixa de revisão `/scans`. Confirmar ou descartar um item apaga sua foto na fila; jobs não resolvidos expiram após 14 dias.
